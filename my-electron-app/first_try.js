function initializeModProteinHeatmap(){
    const options = { selector: '#mod_protein_container', container: '<div id="container"><div id="mod_heatmap"></div></div>' }
    const d3n = new D3Node(options) // initializes D3 with container element
    const d3 = d3n.d3   
    return json("http://127.0.0.1:5000/get-protein-mod-data").catch((err) => {
        console.log(err)
    }).then((data) => {
    ...
    var svg = d3.select(d3n.document.querySelector("#mod_heatmap"))
    .append("svg")
    ...
    // create a tooltip
    var tooltip = d3.select(d3n.document.querySelector("#mod_heatmap"))
    .append("div")
    .style("opacity", 0)
    .attr("class", "tooltip")
